...
public final class BannerUtil {
    ...
    public static BannerMeta emptyBannerMeta() {
        return ((BannerMeta) new org.bukkit.inventory.ItemStack(org.bukkit.Material.LEGACY_BANNER).getItemMeta());
    }
    ...
    public static BannerMeta parseMncPattern(@NotNull String mnc) {
        if (mnc.matches("[a-p]a([a-p][b-zA-M])+")) {
            List<String> groups = StringUtil.sizedSplitString(mnc, 2, true);
    ...
    public static char colorToMnc(@NotNull DyeColor color) {
        for (char c : StringUtil.alphabetLetters()) if (colorFromMnc(c) == color) return c;
        return 0;
    }

    public static char patternTypeToMnc(@NotNull PatternType patternType) {
        for (char c : StringUtil.alphabetLetters())
            if (patternTypeFromMnc(c) == patternType) return c;
        return 0;
    }
