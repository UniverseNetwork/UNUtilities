...
public class CompatibilityUtils {
    ...
    static Class<? extends Enum> soundCategoryClass;
    static final HashMap<String, Method> playSoundMethod = new HashMap<>();
    static float serverVersion = -1;
    ...
    static Class<? extends Enum> getSoundCategoryClass() throws ClassNotFoundException {
    ...
    static Method getPlaySoundMethod(Class sound, boolean soundcategory) throws ClassNotFoundException, NoSuchMethodException {
        Method method = playSoundMethod.get(sound.getName() + soundcategory);
        if (method == null) {
            if (soundcategory)
                method = Player.class.getMethod("playSound", Location.class, sound, getSoundCategoryClass(), float.class, float.class);
            else method = Player.class.getMethod("playSound", Location.class, sound, float.class, float.class);
    ...
    static void playSoundUniversal(Player player, Location location, Object sound, SoundCategory category, float volume, float pitch, float distance) {
        try {
    ...
    public static ArrayList<CustomInstrument> getVersionCustomInstrumentsForSong(int firstCustomInstrumentIndex) {
        ...
        if (getServerVersion() < 0.0112f) {
            if (firstCustomInstrumentIndex == 10) instruments.addAll(getVersionCustomInstruments(0.0112f));
            ...
            if (firstCustomInstrumentIndex == 16) instruments.addAll(getVersionCustomInstruments(0.0114f));
        return instruments;
    }
    ...
    public static float getServerVersion() {
        ...
        for (String part : versionParts) {
            if (part.length() == 1) versionString.append("0");
