...
public class Reflection {
    ...
    final Map<Class<?>, Map<Class<?>, Map<Integer, Field>>> classFields = new HashMap<>();
    ...
    public Class<?> getClass(final String className) {
        if (this.classes.containsKey(className)) return this.classes.get(className);
    ...
    Object getValue(final Field field, final Object object) throws IllegalArgumentException, IllegalAccessException {
    ...
    public Object getField(final Object object, final Class<?> fieldType, final int number) throws IllegalAccessException {
        ...
        if (!fields.isEmpty() && fields.containsKey(number)) return getValue(fields.get(number), object);
        int index = 0;
    ...
    Class<?> getMinecraftClass(String key) {
    ...
    Class<?> getCraftBukkitClass(String key) {
        final int lastDot = key.lastIndexOf(".");
        final String lastKey = key.substring(lastDot > 0 ? lastDot + 1 : 0);
