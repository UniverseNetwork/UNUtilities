...
public final class StackUtils {
    static final NamespacedKey ID_KEY = Slimefun.getItemDataService().getKey();
    ...
    public static boolean isSimilar(@Nullable ItemStack first, @Nullable ItemStack second) {
        ...
        else if (first.hasItemMeta()) if (second.hasItemMeta()) {
            ItemMeta firstMeta = first.getItemMeta();
            ItemMeta secondMeta = second.getItemMeta();
            String firstId = getId(firstMeta);
            if (firstId == null) if (getId(secondMeta) == null) {
        ...
        else return first.getType().equals(second.getType());
    }
}