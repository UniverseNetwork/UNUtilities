...
import id.universenetwork.utilities.Bukkit.Hooks.SlimefunAddons.MobCapturer.Items.MobCannon;
import id.universenetwork.utilities.Bukkit.Hooks.SlimefunAddons.MobCapturer.Items.MobEgg;
import id.universenetwork.utilities.Bukkit.Hooks.SlimefunAddons.MobCapturer.Items.MobPellet;
import id.universenetwork.utilities.Bukkit.Hooks.SlimefunAddons.MobCapturer.Mobs.*;
...
public class MobCapturer {
    final NamespacedKey dataKey = new NamespacedKey(plugin, "captured_mob");
    final NamespacedKey inventoryKey = new NamespacedKey(plugin, "mob_inventory");
    final Map<EntityType, MobEgg<?>> adapters = new EnumMap<>(EntityType.class);
    ItemGroup itemGroup;
    Research research;
    RecipeType recipeType;

    public MobCapturer() {
        if (Enabled("MobCapturer")) {
            new PelletListener(this);
            itemGroup = new ItemGroup(new NamespacedKey(plugin, "mob_capturer"), new CustomItemStack(getItemStack(fromHashCode("d429ff1d2015cb11398471bb2f895f7b4c3ccec201e4ad7a86ff24b744878c")), "&dMob Capturer"));
            research = new Research(new NamespacedKey(plugin, "mob_capturing"), 32652, "Capturing Mobs", 28);
            ...
            mobCannon.register(addon);
            recipeType = new RecipeType(new NamespacedKey(plugin, "mob_capturing"), new CustomItemStack(cannon, "&6Mob Capturing Cannon", "&7Use a &6Mob Capturing Cannon", "&7to catch the given Mob."));
            ...
            register("Wither Skeleton", EntityType.WITHER_SKELETON, new WitherSkeletonAdapter<>(WitherSkeleton.class), "337223d01906ab63af1a15988343b8637e85930b905c35125b545b398c59e1c5");
            register("Stray", EntityType.STRAY, new StrayAdapter<>(Stray.class), "5b45aae241779f0617ffaff468f3f2cf666d2f8a803002f9ae1ba0f14ed79fdd");
            ...
            research.register();
            System.out.println(prefix + " §bSuccessfully Registered §dMobCapturer §bAddon");
        }
    }
    ...
    public <T extends LivingEntity> void register(String name, EntityType type, MobAdapter<T> adapter, String eggTexture) {
        ...
        MobEgg<T> egg = new MobEgg<>(itemGroup, itemstack, dataKey, inventoryKey, adapter, recipeType, new ItemStack[]{null, null, null, null, new CustomItemStack(getItemStack(fromHashCode(eggTexture)), WHITE + name), null, null, null, null});
        egg.register(addon);
        if (!egg.isDisabled()) {
