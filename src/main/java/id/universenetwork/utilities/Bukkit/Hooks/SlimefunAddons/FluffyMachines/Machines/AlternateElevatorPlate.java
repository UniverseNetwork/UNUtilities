...
public class AlternateElevatorPlate extends SimpleSlimefunItem<BlockUseHandler> {
    static final String DATA_KEY = "floor";
    final Set<UUID> users = new HashSet<>();
    static final int MAX_CHEST_INDEX = 53;

    public AlternateElevatorPlate(ItemGroup itemGroup, SlimefunItemStack item, RecipeType recipeType, ItemStack[] recipe, ItemStack recipeOutput) {
    ...
    @NotNull
    public List<Block> getFloors(@NotNull Block b) {
    ...
    @ParametersAreNonnullByDefault
    void openFloorSelector(Block b, List<Block> floors, Player p) {
    ...
    @ParametersAreNonnullByDefault
    void teleport(Player player, String floorName, Block target) {
        Utils.runSync(() -> {
    ...
    void addFloor(ChestMenu menu, int slot, Player p, String floor, Block b, Block destination) {
        if (destination.getY() == b.getY()) {
            menu.addItem(slot, new CustomItemStack(Material.LIME_STAINED_GLASS_PANE, ChatColors.color(Slimefun.getLocalization().getMessage(p, "machines.ELEVATOR.current-floor")), "", ChatColor.WHITE + floor, ""));
            menu.addMenuClickHandler(slot, ChestMenuUtils.getEmptyClickHandler());
        } else {
            menu.addItem(slot, new CustomItemStack(Material.GRAY_STAINED_GLASS_PANE, ChatColors.color(Slimefun.getLocalization().getMessage(p, "machines.ELEVATOR.click-to-teleport")), "", ChatColor.WHITE + floor, ""));
