...
public class ExprTameOwner extends SimplePropertyExpression<Entity, Player> {
    ...
    public Class<?>[] acceptChange(Changer.ChangeMode mode) {
        if (mode == Changer.ChangeMode.RESET || mode == Changer.ChangeMode.SET) return asArray(Player.class);
        return null;
    }
    ...
    public void change(Event e, Object[] delta, Changer.ChangeMode mode) {
        ...
        switch (mode) {
            case SET:
                for (Entity en : getExpr().getAll(e)) if (en instanceof Tameable) ((Tameable) en).setOwner(n);
                break;
            case RESET:
                for (Entity en : getExpr().getAll(e)) if (en instanceof Tameable) ((Tameable) en).setOwner(null);
            case ADD:
            case REMOVE:
            case REMOVE_ALL:
            case DELETE:
