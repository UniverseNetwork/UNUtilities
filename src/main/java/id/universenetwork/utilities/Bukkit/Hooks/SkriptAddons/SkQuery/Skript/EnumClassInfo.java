...
public class EnumClassInfo<E extends Enum<E>> {
    ...
    final ClassInfo<E> classInfo;
    final String codeName;
    final String user;
    final HashMap<String, String> synonyms = new HashMap<>();

    EnumClassInfo(Class<E> enumType, String codeName, @Nullable String user) {
                ...
                public E parse(String s, ParseContext parseContext) {
                    ...
                    try {
                        for (Map.Entry<String, String> p : synonyms.entrySet())
                            if (s.matches(p.getKey())) return E.valueOf(enumType, p.getValue());
