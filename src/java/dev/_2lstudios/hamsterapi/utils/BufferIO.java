...
public class BufferIO {
    ...
    public ByteBuf split(final ByteBuf bytebuf) throws DecoderException, IOException, IllegalAccessException, InvocationTargetException, NoSuchMethodException, InstantiationException, NoSuchFieldException {
        ...
        for (int i = 0; i < abyte.length; ++i) {
            ...
            if (abyte[i] >= 0) {
                final Object packetDataSerializer = packetDataSerializerClass.getConstructor(ByteBuf.class).newInstance(Unpooled.wrappedBuffer(abyte));
                try {
    ...
    public ByteBuf decompress(final ByteBuf byteBuf) throws DecoderException, IllegalAccessException, InvocationTargetException, NoSuchMethodException, InstantiationException, DataFormatException {
    ...
    public PacketWrapper decode(final ChannelHandlerContext chx, final ByteBuf byteBuf, final int maxCapacity) throws DecoderException, IOException, IllegalAccessException, InvocationTargetException, NoSuchMethodException, InstantiationException, NoSuchFieldException {
        if (byteBuf.readableBytes() != 0) {
            ...
            final Object packetDataSerializer = packetDataSerializerClass.getConstructor(ByteBuf.class).newInstance(byteBuf);
            ...
            final Object packet = enumProtocolClass.getMethod("a", enumProtocolDirectionClass, int.class).invoke(enumProtocolClass.cast(attribute), enumProtocolDirectionClass.getField("SERVERBOUND").get(null), id);
