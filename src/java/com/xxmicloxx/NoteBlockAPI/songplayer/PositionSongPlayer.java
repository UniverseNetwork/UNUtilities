...
public class PositionSongPlayer extends RangeSongPlayer {
    ...
    public void playTick(Player player, int tick) {
        if (!player.getWorld().getName().equals(targetLocation.getWorld().getName())) return; // not in same world
        ...
        for (Layer layer : song.getLayerHashMap().values()) {
            ...
            if (isInRange(player)) {
                if (!playerList.get(player.getUniqueId())) {
                    ...
                    plugin.doSync(() -> getPluginManager().callEvent(new PlayerRangeStateChangeEvent(this, player, true)));
                }
            } else {
                if (playerList.get(player.getUniqueId())) {
                    playerList.put(player.getUniqueId(), false);
                    plugin.doSync(() -> getPluginManager().callEvent(new PlayerRangeStateChangeEvent(this, player, false)));
                }
            }
        }
    }
