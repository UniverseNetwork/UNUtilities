...
public class NBSDecoder {
     ...
     * @see Song
     */
    static Song parse(InputStream inputStream, File songFile) {
        ...
        try {
            ...
            while (true) {
                ...
                while (true) {
                    ...
                    if (panning != 100) isStereo = true;
                    setNote(layer, tick, new Note(instrument /* instrument */, key/* note */, velocity, panning, pitch), layerHashMap);
                }
            }
            ...
            } else firstcustominstrument += firstcustominstrumentdiff;
            return new Song(speed, layerHashMap, songHeight, length, title, author, originalAuthor, description, songFile, firstcustominstrument, customInstrumentsArray, isStereo);
        } catch (EOFException e) {
            String file = "";
            if (songFile != null) file = songFile.getName();
            getServer().getConsoleSender().sendMessage(ChatColor.RED + "Song is corrupted: " + file);
        } catch (IOException e) {
            e.printStackTrace();
        }
        return null;
     ...
     */
    static void setNote(int layerIndex, int ticks, Note note, HashMap<Integer, Layer> layerHashMap) {
