...
public class Playlist {
    ...
    public Playlist(Song... songs) {
        if (songs.length == 0) throw new IllegalArgumentException("Cannot create empty playlist");
        checkNull(songs);
    ...
    public void insert(int index, Song... songs) {
        if (songs.length == 0) return;
        if (index > this.songs.size()) throw new IllegalArgumentException("Index is higher than playlist size");
        checkNull(songs);
        this.songs.addAll(index, Arrays.asList(songs));
    }

    void checkNull(Song... songs) {
        List<Song> songList = Arrays.asList(songs);
        if (songList.contains(null)) throw new IllegalArgumentException("Cannot add null to playlist");
    }
    ...
    public void remove(Song... songs) {
        ...
        if (songsTemp.size() > 0) this.songs = songsTemp;
        else throw new IllegalArgumentException("Cannot remove all songs from playlist");
    }
