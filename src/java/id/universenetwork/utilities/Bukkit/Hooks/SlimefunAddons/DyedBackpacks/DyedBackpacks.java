 ...
 * @see DyedBackpack
 */
public class DyedBackpacks extends id.universenetwork.utilities.Bukkit.Hooks.SlimefunAddons.SFInstance {
    @Override
    public void Load() {
        Research research = new Research(createKey("dyed_backpacks"), 17200, "Dyed Backpacks", 24);
        ItemGroup itemGroup = new ItemGroup(createKey("dyed_backpacks"), new io.github.thebusybiscuit.slimefun4.libraries.dough.items.CustomItemStack(io.github.thebusybiscuit.slimefun4.libraries.dough.skins.PlayerHead.getItemStack(io.github.thebusybiscuit.slimefun4.libraries.dough.skins.PlayerSkin.fromHashCode(BackpackColor.RED.getTexture())), "&4Dyed Backpacks"), 2);
        if (config.getBoolean(configPath + "backpacks.small")) createBackpacks(itemGroup, research, BACKPACK_SMALL, 9);
        if (config.getBoolean(configPath + "backpacks.normal"))
            createBackpacks(itemGroup, research, BACKPACK_MEDIUM, 18);
        if (config.getBoolean(configPath + "backpacks.large")) createBackpacks(itemGroup, research, BACKPACK_LARGE, 27);
        if (config.getBoolean(configPath + "backpacks.woven")) createBackpacks(itemGroup, research, WOVEN_BACKPACK, 36);
        if (config.getBoolean(configPath + "backpacks.gilded"))
            createBackpacks(itemGroup, research, GILDED_BACKPACK, 45);
        if (config.getBoolean(configPath + "backpacks.radiant"))
