...
import id.universenetwork.utilities.Bukkit.Libraries.JNBT.*;
...
import static me.mrCookieSlime.Slimefun.api.BlockStorage.store;
/*
...
public class Schematic {
    ...
    public static void pasteSchematic(Location loc, Tree tree) {
        ...
        try {
            ...
            org.bukkit.Bukkit.getLogger().log(java.util.logging.Level.WARNING, id.universenetwork.utilities.Bukkit.UNUtilities.prefix + "Could not paste Schematic for Tree: " + tree.getFruitID() + "_TREE (" + e.getClass().getSimpleName() + ')', e);
            return;
        }
        ...
        for (int x = 0; x < width; ++x)
            for (int y = 0; y < height; ++y)
                for (int z = 0; z < length; ++z) {
                    ...
                    org.bukkit.block.Block block = new Location(loc.getWorld(), blockX, blockY, blockZ).getBlock();
                    Material blockType = block.getType();
                    if ((!blockType.isSolid() && !blockType.isInteractable() && !io.github.thebusybiscuit.slimefun4.utils.tags.SlimefunTag.UNBREAKABLE_MATERIALS.isTagged(blockType)) || blockType == Material.AIR || blockType == Material.CAVE_AIR || org.bukkit.Tag.SAPLINGS.isTagged(blockType)) {
                        ...
                        if (material != null) {
                            ...
                            if (org.bukkit.Tag.LEAVES.isTagged(material)) {
                                if (current().nextInt(100) < 25) store(block, tree.getItem());
                                ...
                                io.github.thebusybiscuit.slimefun4.libraries.dough.skins.PlayerHead.setSkin(block, io.github.thebusybiscuit.slimefun4.libraries.dough.skins.PlayerSkin.fromHashCode(tree.getTexture()), true);
                                store(block, tree.getFruit());
                            }
                        }
                    }
                }
    ...
    public static Schematic loadSchematic(File file) throws IOException {
        ...
        try (NBTInputStream stream = new NBTInputStream(new java.io.FileInputStream(file))) {
        ...
        for (int index = 0; index < blockId.length; index++) {
            ...
            if ((index >> 1) >= addId.length) blocks[index] = (short) (blockId[index] & 0xFF);
            else {
                if ((index & 1) == 0)
