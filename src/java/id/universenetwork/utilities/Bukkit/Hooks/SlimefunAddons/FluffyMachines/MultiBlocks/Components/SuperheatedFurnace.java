...
public class SuperheatedFurnace extends NonHopperableBlock {
    static final int[] inputBorder = {0, 2, 9, 11, 18, 19, 20};
    static final int[] dustOutputBorder = {3, 5, 12, 14, 21, 22, 23};
    static final int[] ingotOutputBorder = {6, 8, 15, 17, 24, 25, 26};
    static final int INPUT_SLOT = 10;
    static final int DUST_OUTPUT_SLOT = 13;
    static final int INGOT_OUTPUT_SLOT = 16;
    static final int INPUT_INDICATOR = 1;
    static final int DUST_INDICATOR = 4;
    static final int INGOT_INDICATOR = 7;
    static final int MAX_STORAGE = 138240;
    static final Material netherite = Material.NETHERITE_BLOCK;
    final int MAX_STACK_SIZE = 64;
    static final SlimefunItemStack[] dusts = new SlimefunItemStack[]{SlimefunItems.COPPER_DUST, SlimefunItems.GOLD_DUST, SlimefunItems.IRON_DUST, SlimefunItems.LEAD_DUST, SlimefunItems.ALUMINUM_DUST, SlimefunItems.ZINC_DUST, SlimefunItems.TIN_DUST, SlimefunItems.SILVER_DUST, SlimefunItems.MAGNESIUM_DUST};
    static final SlimefunItemStack[] ingots = new SlimefunItemStack[]{SlimefunItems.COPPER_INGOT, SlimefunItems.LEAD_INGOT, SlimefunItems.ALUMINUM_INGOT, SlimefunItems.ZINC_INGOT, SlimefunItems.TIN_INGOT, SlimefunItems.SILVER_INGOT, SlimefunItems.MAGNESIUM_INGOT};
    ...
    public SuperheatedFurnace(ItemGroup itemGroup, SlimefunItemStack item, RecipeType recipeType, ItemStack[] recipe) {
        ...
        new BlockMenuPreset(getId(), "&cFoundry") {
            ...
            public boolean canOpen(@NotNull Block b, @NotNull Player p) {
                return (p.hasPermission("slimefun.inventory.bypass") || Slimefun.getProtectionManager().hasPermission(p, b.getLocation(), Interaction.INTERACT_BLOCK)) && getBlockInfo(b.getLocation(), "accessible") != null && getBlockInfo(b.getLocation(), "ignited") != null && checkStructure(b);
            }
                ...
                if (inv != null) {
                    ...
                    if (stored > 0) {
                        ...
                        if (stored > OVERFLOW_AMOUNT) {
                            Utils.send(p, "&eThere are more than " + OVERFLOW_AMOUNT + " items in this superheated furnace! " + "Dropping " + OVERFLOW_AMOUNT + " items instead!");
    ...
    protected void tick(Block b) {
        ...
        if (inputItem != null) {
            ...
            if (type == null) {
                ...
                if (sfItem != null && ((sfItem.getId().equals(type + "_DUST") || sfItem.getId().equals(type + "_INGOT")) || (type.equals("GOLD") && sfItem.getId().equals(SlimefunItems.GOLD_4K.getItemId()))) || (type.equals("IRON") && inputItem.getType() == Material.IRON_INGOT && inputItem.getItemMeta().equals(new ItemStack(Material.IRON_INGOT).getItemMeta())) && stored + amount < MAX_STORAGE) {
                    inv.consumeItem(INPUT_SLOT, amount);
                    addDust(b, amount);
                }
            }
    ...
    void registerDust(Block b, String type, int amount) {
        int stored = Integer.parseInt(getBlockInfo(b.getLocation(), "stored"));
    ...
    void addDust(Block b, int amount) {
        int stored = Integer.parseInt(getBlockInfo(b.getLocation(), "stored"));
        setBlockInfo(b, "stored", String.valueOf(stored + amount));
        updateIndicator(b);
    }

    void updateIndicator(Block b) {
        BlockMenu inv = BlockStorage.getInventory(b);
    ...
    void retrieveDust(BlockMenu menu, Block b, boolean isRightClicked) {
        if (getBlockInfo(b.getLocation(), "stored") == null) return;
        int stored = Integer.parseInt(getBlockInfo(b.getLocation(), "stored"));
        if (stored > 0 && (menu.getItemInSlot(DUST_OUTPUT_SLOT) == null || menu.getItemInSlot(DUST_OUTPUT_SLOT).getAmount() < 64)) {
            String type = getBlockInfo(b.getLocation(), "type");
            int amount;
    ...
    void retrieveIngot(BlockMenu menu, Block b, boolean isRightClicked) {
        if (getBlockInfo(b.getLocation(), "stored") == null) return;
        int stored = Integer.parseInt(getBlockInfo(b.getLocation(), "stored"));
        if (stored > 0 && (menu.getItemInSlot(INGOT_OUTPUT_SLOT) == null || menu.getItemInSlot(INGOT_OUTPUT_SLOT).getAmount() < 64)) {
            String type = getBlockInfo(b.getLocation(), "type");
            int amount;
            if (!isRightClicked) amount = 1;
            else if (stored < MAX_STACK_SIZE) amount = stored;
    ...
    boolean checkStructure(Block b) {
        BlockFace face;
        Block relative;
        ...
        return b.getRelative(face).getType() == netherite && checkRite(relative.getRelative(0, -1, 0)) && checkRite(relative.getRelative(0, -2, 0)) && checkRite(b.getRelative(face.getOppositeFace()).getRelative(0, -1, 0)) && checkRite(b.getRelative(face.getOppositeFace()).getRelative(0, -2, 0)) && b.getRelative(0, -1, 0).getType() == Material.GLASS && b.getRelative(0, -2, 0).getType() == Material.CAULDRON;
    }

    boolean checkRite(Block b) {
        return (b.getType() == netherite);
    }

    void setBlockInfo(Block b, String key, String data) {
        BlockStorage.addBlockInfo(b, key, data);
    }

    String getBlockInfo(Location l, String key) {
